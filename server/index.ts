...
// Wire collaboration events to WebSocket broadcast
collaborationManager.onEvent((event) => {
  broadcast({ type: 'collaboration_event', payload: event })
})
...
function handleHttpRequest(req: IncomingMessage, res: ServerResponse) {
  ...
  if (req.url?.startsWith('/api/collaboration/')) {
    ...
    if (req.method === 'POST' && action === 'request') {
      collectRequestBody(req).then(body => {
        try {
          const { fromAgent, toAgent, task } = JSON.parse(body) as { fromAgent: string; toAgent: string; task: string }
          const request = collaborationManager.requestCollaboration(fromAgent, toAgent, task)
          res.writeHead(200, { 'Content-Type': 'application/json' })
    ...
    if (req.method === 'POST' && action === 'accept') {
      collectRequestBody(req).then(body => {
        try {
          ...
          const success = collaborationManager.acceptCollaboration(requestId)
          res.writeHead(200, { 'Content-Type': 'application/json' })
    ...
    if (req.method === 'POST' && action === 'reject') {
      collectRequestBody(req).then(body => {
        try {
          const { requestId, reason } = JSON.parse(body) as { requestId: string; reason?: string }
          const success = collaborationManager.rejectCollaboration(requestId, reason)
          res.writeHead(200, { 'Content-Type': 'application/json' })
          res.end(JSON.stringify({ ok: success }))
        } catch (e) {
          res.writeHead(400, { 'Content-Type': 'application/json' })
  ...
  // ============================================================================
  // Ralph API (autonomous agent control)
  // ============================================================================

  // Start Ralph agent
  if (req.method === 'POST' && req.url === '/ralph/start') {
    collectRequestBody(req).then(async body => {
      try {
        ...
        const ralphCwd = resolve(expandHome(data.cwd || process.cwd()))
        const ralphScriptPath = join(ralphCwd, 'scripts/ralph/ralph.sh')

        // Check if ralph.sh exists
        if (!existsSync(ralphScriptPath)) {
          res.writeHead(404, { 'Content-Type': 'application/json' })
          res.end(JSON.stringify({
            ok: false,
            error: `Ralph script not found at ${ralphScriptPath}`
          }))
          return
        }

        // Spawn Ralph process in background
        exec(
          `cd "${ralphCwd}" && ./scripts/ralph/ralph.sh ${maxIterations} > /tmp/ralph-output.log 2>&1 &`,
          EXEC_OPTIONS,
          (error) => {
            if (error) {
              log(`Ralph start error: ${error.message}`)
            }
          }
        )

        log(`Ralph started: ${ralphScriptPath} (max ${maxIterations} iterations)`)
        res.writeHead(200, { 'Content-Type': 'application/json' })
        res.end(JSON.stringify({
          ok: true,
  ...
  // Stop Ralph agent
  if (req.method === 'POST' && req.url === '/ralph/stop') {
    collectRequestBody(req).then(async body => {
      try {
        const data = body ? JSON.parse(body) : {}
        ...
        // Note: This is a simple implementation - in production you'd want to track PIDs
        exec(
          `pkill -f "ralph.sh"`,
          EXEC_OPTIONS,
          (error, stdout, stderr) => {
            if (error && error.code !== 1) { // code 1 means no matching processes
              log(`Ralph stop error: ${error.message}`)
              res.writeHead(500, { 'Content-Type': 'application/json' })
              res.end(JSON.stringify({ ok: false, error: error.message }))
              return
            }

            log(`Ralph stopped for session: ${sessionId}`)
            res.writeHead(200, { 'Content-Type': 'application/json' })
            res.end(JSON.stringify({ ok: true, message: 'Ralph stopped' }))
          }
        )
      } catch (error: any) {
        res.writeHead(400, { 'Content-Type': 'application/json' })
  ...
  // POST /reactor/event - ZENITH: Handle events from the backend orchestrator
  if (req.method === 'POST' && req.url === '/reactor/event') {
    collectRequestBody(req).then(body => {
      try {
        const payload = JSON.parse(body)
        broadcast({ type: 'reactor_event', payload })
        res.writeHead(200, { 'Content-Type': 'application/json' })
        res.end(JSON.stringify({ ok: true }))
      } catch (e) {
        res.writeHead(400, { 'Content-Type': 'application/json' })
...
function main() {
  ...
  wss.on('connection', (ws, req) => {
    ...
    const filteredHistory = events
      .filter(e => activeClaudeSessionIds.has(e.sessionId) || (e as any).agentId)
      .slice(-100)
