...
export class FeedManager {
  ...
  /**
   * Set the AgentManager for multi-agent attribution
   */
  ...
  setFilter(sessionId: string | null): void {
    ...
    this.feedEl.querySelectorAll('.feed-item').forEach((item) => {
      ...
      const itemAgent = itemEl.dataset.agentId

      // Show all if no filter, or show matching session OR matching agent
      // Note: "session_agentId" is the convention used in multiAgentInit.ts
      const isMatch = sessionId === null ||
        itemSession === sessionId ||
        (sessionId.startsWith('session_') && sessionId === `session_${itemAgent}`)
  ...
  add(event: ClaudeEvent, sessionColor?: number): void {
    ...
    item.dataset.agentId = (event as any).agentId

    // Customize based on agent if available
    let agentName = 'Claude'
    let agentAvatar = '🤖'
    let borderColor = sessionColor

    // Check optional agentId property on event (added in types.ts)
    const agentId = (event as any).agentId
    if (agentId && this.agentManager) {
      const agent = this.agentManager.getAgent(agentId)
      if (agent) {
        agentName = agent.getName()
        agentAvatar = agent.getType() === 'human' ? '👤' :
          agent.getId() === 'antigravity' ? '🌌' :
            agent.getId() === 'jules' ? '🔥' :
              agent.getId() === 'ollama' ? '🦙' : '🤖'

        // Use agent color if available, otherwise session color
        const color = agent.getColor()
        if (color) {
          borderColor = parseInt(color.replace('#', ''), 16)
        }
      }
    }
    ...
    if (borderColor !== undefined) {
      item.style.borderLeftColor = `#${borderColor.toString(16).padStart(6, '0')}`
      item.style.borderLeftWidth = '3px'
      item.style.borderLeftStyle = 'solid'
    }
    ...
    switch (event.type) {
      ...
      case 'stop': {
        ...
        if (response) {
          ...
          item.innerHTML = `
            <div class="feed-item-header">
              <div class="feed-item-icon">${agentAvatar}</div>
    ...
    if (this.activeFilter !== null) {
      const itemAgent = (event as any).agentId
      const isMatch = event.sessionId === this.activeFilter ||
        (this.activeFilter.startsWith('session_') && this.activeFilter === `session_${itemAgent}`)

      if (!isMatch) item.style.display = 'none'
