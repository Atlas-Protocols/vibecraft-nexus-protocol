...
// TELEMETRY DISABLED: import { checkForUpdates } from './ui/VersionChecker'
import { drawMode } from './ui/DrawMode'
import { setupTextLabelModal, showTextLabelModal } from './ui/TextLabelModal'
import { RalphMonitor } from './ui/RalphMonitor'
...
interface AppState {
  ...
  ralphMonitor: RalphMonitor | null  // Manages Ralph autonomous agent monitoring
...
const state: AppState = {
  ...
  ralphMonitor: null,  // Initialized in init()
  soundEnabled: true,
...
function handleEvent(event: ClaudeEvent) {
  ...
  // Handle Ralph events
  state.ralphMonitor?.handleEvent(event)
...
async function init() {
  ...
  state.ralphMonitor = new RalphMonitor(API_URL)
  ...
  // TELEMETRY DISABLED: Check for updates (non-blocking)
  // checkForUpdates()

  // Initialize multi-agent system (Antigravity, Jules, Ollama)
  if (state.scene) {
    const { agentManager } = await initMultiAgentSystem(state.scene.scene)  // Pass Three.js Scene, not WorkshopScene
    if (state.feedManager) {
      state.feedManager.setAgentManager(agentManager)
    }
  }
