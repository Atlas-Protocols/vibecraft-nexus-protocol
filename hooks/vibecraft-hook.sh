...
# Extract Agent ID with fallback order:
# 1. agent_id from JSON (if passed by specific agent)
# 2. VIBECRAFT_AGENT_ID env var (if set globally/per-process)
# 3. Default to "business-os" (legacy/default behavior)
raw_agent_id=$(echo "$input" | "$JQ" -r '.agent_id // ""')
if [ -n "$raw_agent_id" ]; then
  agent_id="$raw_agent_id"
elif [ -n "$VIBECRAFT_AGENT_ID" ]; then
  agent_id="$VIBECRAFT_AGENT_ID"
else
  agent_id="business-os"
fi
