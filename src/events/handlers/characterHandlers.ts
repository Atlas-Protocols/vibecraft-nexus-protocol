...
export function registerCharacterHandlers(): void {
  ...
  eventBus.on('pre_tool_use', (event: PreToolUseEvent, ctx) => {
    // Check for multi-agent event
    if (event.agentId && event.agentId !== 'business-os' && ctx.scene?.multiAgentScene) {
      // Delegate to MultiAgentScene
      // Note: We need to cast or access the method defined in multiAgentInit.ts
      // Since EventContext doesn't typing for multiAgentScene, we check availability
      const maScene = ctx.scene.multiAgentScene as any
      if (maScene && typeof maScene.updateCharacter === 'function') {
        maScene.updateCharacter(event.agentId, 'working')
        // Also pulse their zone
        if (typeof maScene.pulseZone === 'function') {
          const station = getStationForTool(event.tool)
          // Use station mapping if needed, or just general pulse
          maScene.pulseZone(event.agentId)
        }
      }
      return
    }
  ...
  eventBus.on('post_tool_use', (event: PostToolUseEvent, ctx) => {
    ...
    if (event.agentId && event.agentId !== 'business-os' && ctx.scene?.multiAgentScene) {
      const maScene = ctx.scene.multiAgentScene as any
      if (maScene && typeof maScene.updateCharacter === 'function') {
        maScene.updateCharacter(event.agentId, 'idle') // or 'thinking' if followed by output
      }
      return
    }
  ...
  eventBus.on('stop', (event: StopEvent, ctx) => {
    ...
    if (event.agentId && event.agentId !== 'business-os' && ctx.scene?.multiAgentScene) {
      const maScene = ctx.scene.multiAgentScene as any
      if (maScene && typeof maScene.updateCharacter === 'function') {
        maScene.updateCharacter(event.agentId, 'idle')
      }
      return
    }
  ...
  eventBus.on('user_prompt_submit', (event: UserPromptSubmitEvent, ctx) => {
    ...
    if (event.agentId && event.agentId !== 'business-os' && ctx.scene?.multiAgentScene) {
      const maScene = ctx.scene.multiAgentScene as any
      if (maScene && typeof maScene.updateCharacter === 'function') {
        maScene.updateCharacter(event.agentId, 'thinking')
      }
      return
    }
